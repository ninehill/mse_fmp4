...
pub fn to_fmp4<R: ReadTsPacket>(reader: R) -> Result<(InitializationSegment, MediaSegment)> {
    let avc_stream = track!(read_avc_stream(reader))?;
    let initialization_segment = track!(make_initialization_segment(&avc_stream))?;
    let media_segment = track!(make_media_segment(avc_stream))?;
    Ok((initialization_segment, media_segment))
}

fn make_initialization_segment(avc_stream: &AvcStream) -> Result<InitializationSegment> {
...
fn make_media_segment(avc_stream: AvcStream) -> Result<MediaSegment> {
...
fn read_avc_stream<R: ReadTsPacket>(ts_reader: R) -> Result<AvcStream> {
